...
void main() {
    ...
    while(1) {
        ...
        if (doorButton()) {
            if ((level() == 1) && !PORTBbits.RB0 && !PORTBbits.RB1) {
                maintenance();
                lcd_cmd(0x01);
            }
...
int level(){
    unsigned int floor;
    adc_value = read_adc();
    scaled_adc = (float)adc_value * 6/1023*1000-200;
...
int floorSelect() {
    if (PORTBbits.RB6 || PORTBbits.RB7 || PORTAbits.RA6) {
        return 1;
    }
    else {
        return 0;
...
void motorControl(int spd) {
    int PWMfreq = 2500;
    int ontime = (spd*(20000000/PWMfreq))/160;
...
void delay(int time) {
    int initial_time = timer0_count;
    while (timer0_count - initial_time < time);
}
...
int elevatorMoving(int flr){
    ...
    else if ((scaled_adc > 5200 && scaled_adc < 5800) || (scaled_adc > 2400 && scaled_adc < 3200) || (scaled_adc > -400 && scaled_adc < 400)) {
        motorControl(2);
        return 1;
    }
    else {
...
void elevatorStart(int destination) {
    ...
    if((PORTAbits.RA1 == 1 && level() == 2 && destination < level()) || (PORTAbits.RA1 == 0 && level() == 2 && destination > level())){
        return;
    }
    doorControl(2);
    while(!doorAjar()) {
    }
    while(elevatorMoving(destination)) {
    }
}
